...
void Test_MY_APP_ReportHousekeeping(void)
{
    ...
    MY_APP_SendHkCmd(NULL);

    /* Confirm message sent*/
    UtAssert_STUB_COUNT(CFE_SB_TransmitMsg, 1);
    UtAssert_ADDRESS_EQ(MsgSend, &MY_APP_Data.HkTlm);

    /* Confirm timestamp msg address */
    UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
    UtAssert_ADDRESS_EQ(MsgTimestamp, &MY_APP_Data.HkTlm);
...
void Test_MY_APP_NoopCmd(void)
{
    ...
    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_NOOP_INF_EID, NULL);

    UtAssert_INT32_EQ(MY_APP_NoopCmd(&TestMsg), CFE_SUCCESS);
...
void Test_MY_APP_ResetCountersCmd(void)
{
    /*
     * Test Case For:
    ...
    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_RESET_INF_EID, "SAMPLE: RESET command");

    UtAssert_INT32_EQ(MY_APP_ResetCountersCmd(&TestMsg), CFE_SUCCESS);
...
void Test_MY_APP_ProcessCmd(void)
{
    /*
     * Test Case For:
     * void  MY_APP_ProcessCmd( const MY_APP_ProcessCmd_t *Msg )
     */
    MY_APP_ProcessCmd_t   TestMsg;
    MY_APP_ExampleTable_t TestTblData;
    ...
    UtAssert_INT32_EQ(MY_APP_ProcessCmd(&TestMsg), CFE_SUCCESS);
     ...
     * MY_APP_GetCrc() function is a stub.
     */
    ...
    UtAssert_INT32_EQ(MY_APP_ProcessCmd(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
    ...
    UtAssert_INT32_EQ(MY_APP_ProcessCmd(&TestMsg), CFE_TBL_ERR_NO_ACCESS);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 4);
}

void Test_MY_APP_DisplayParamCmd(void)
{
    /*
     * Test Case For:
     * void  MY_APP_DisplayParamCmd( const MY_APP_DisplayParamCmd_t *Msg )
     */
    MY_APP_DisplayParamCmd_t TestMsg;
    UT_CheckEvent_t              EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_VALUE_INF_EID, "MY_APP: ValU32=%lu, ValI16=%d, ValStr=%s");
    TestMsg.Payload.ValU32 = 10;
    TestMsg.Payload.ValI16 = -4;
    snprintf(TestMsg.Payload.ValStr, sizeof(TestMsg.Payload.ValStr), "Hello");

    UtAssert_INT32_EQ(MY_APP_DisplayParamCmd(&TestMsg), CFE_SUCCESS);
    /*
     * Confirm that the event was generated
     */
    UtAssert_UINT32_EQ(EventTest.MatchCount, 1);
...
void UtTest_Setup(void)
{
    ...
    ADD_TEST(MY_APP_ResetCountersCmd);
    ADD_TEST(MY_APP_ProcessCmd);
    ADD_TEST(MY_APP_DisplayParamCmd);
}
