...
void Test_MY_APP_Main(void)
{
     ...
     */
    UtAssert_UINT32_EQ(MY_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_PIPE_ERR_EID, "SAMPLE APP: SB Pipe Read Error, App Will Exit");
...
void Test_MY_APP_Init(void)
{
    ...
    UtAssert_INT32_EQ(MY_APP_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(MY_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(MY_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(MY_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
