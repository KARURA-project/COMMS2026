...
void Test_MY_APP_TaskPipe(void)
{
    /*
     * Test Case For:
     * void MY_APP_TaskPipe
     */
    ...
    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_MID_ERR_EID, "SAMPLE: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(MY_APP_CMD_MID);
    FcnCode   = MY_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    MY_APP_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(MY_APP_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MY_APP_TaskPipe(&TestMsg.SBBuf);
...
void Test_MY_APP_ProcessGroundCommand(void)
{
    ...
    union
    {
        ...
        MY_APP_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_CMD_LEN_ERR_EID,
    ...
    UtAssert_STUB_COUNT(MY_APP_NoopCmd, 1);
    ...
    UtAssert_STUB_COUNT(MY_APP_ResetCountersCmd, 1);
    ...
    UtAssert_STUB_COUNT(MY_APP_ProcessCmd, 1);
    ...
    FcnCode = MY_APP_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(MY_APP_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, MY_APP_CC_ERR_EID, "Invalid ground command code: CC = %d");
    MY_APP_ProcessGroundCommand(&TestMsg.SBBuf);
